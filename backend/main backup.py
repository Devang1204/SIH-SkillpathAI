...
ROADMAP_GEMINI_API_KEY = os.getenv("ROADMAP_GEMINI_API_KEY")

if ROADMAP_GEMINI_API_KEY:
    try:
        roadmap_gemini_client = genai.Client(
            api_key=ROADMAP_GEMINI_API_KEY
        )
    except Exception:
        roadmap_gemini_client = None
...
def generate_roadmap_with_ai(
    ...
    if not roadmap_gemini_client:
        return {
            "status": "ai_not_configured",
            "message": (
                "ROADMAP_GEMINI_API_KEY is not configured "
                "in the backend environment."
            ),
        }

    prompt = f"""
Create a personalized learning roadmap for the target role below.
...
Skills the student needs to develop:
{json.dumps(missing_skills)}

IMPORTANT RULES:
- Focus the roadmap primarily on the missing skills.
- Do not create learning tasks for skills the student already has.
- Arrange the missing skills in a logical learning order.
- Start with foundational skills before advanced skills.
- Include practical learning and projects.
- Make the roadmap suitable for a student.
- Do not invent skills outside the provided skill gaps unless they are
  necessary as a prerequisite for one of the provided gaps.
- Return ONLY valid JSON.
- Do not use Markdown.
...
"""
    ...
    try:
        response = roadmap_gemini_client.models.generate_content(
            model="gemini-3.6-flash",
            contents=prompt,
        )
...
def create_roadmap(
    ...
    if request.target_role not in roles:
        raise HTTPException(
            status_code=404,
            detail=(
                f"Role '{request.target_role}' is not available. "
                f"Available roles: {list(roles.keys())}"
            ),
        )
